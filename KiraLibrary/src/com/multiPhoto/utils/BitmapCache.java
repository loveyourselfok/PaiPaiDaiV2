	...
	public void displayBmp(final ImageView iv, final String thumbPath,
		...
		new Thread() {
			...
			public void run() {
				...
				put(path, thumb);
				// int degree = PictureUtil.readPictureDegree(sourcePath);
				// thumb = PictureUtil.rotaingImageView(degree, thumb);
				if (callback != null) {
					h.post(new Runnable() {
						@Override
						public void run() {
							// int degree =
							// PictureUtil.readPictureDegree(sourcePath);
							// thumb = PictureUtil.rotaingImageView(degree,
							// thumb);
